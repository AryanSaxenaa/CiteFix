...
export async function generateBriefDocx(job: AnalysisJob): Promise<Buffer> {
  ...
  sections.push(
    new Paragraph({
      children: [
        new TextRun({ text: "Scoutlytics Implementation Brief", bold: true, size: 48, color: BRAND_COLOR, font: "Segoe UI" }),
      ],
      spacing: { after: 100 },
    }),
    new Paragraph({
  ...
  sections.push(
    bodyText(
      `This implementation brief provides deployment-ready assets to improve ${domain}'s visibility in AI-generated answers for the topic "${topic}". Based on live citation analysis of ${citedUrls.length} top-cited pages, Scoutlytics identified ${gaps.length} actionable gaps and generated the fixes needed to close them.`
    )
  );
  ...
  const checklistItems = [
    ...
    "Re-run Scoutlytics analysis to verify improvement",
  ];
  ...
  const doc = new Document({
    creator: "Scoutlytics",
    title: `Scoutlytics Implementation Brief — ${domain}`,
          ...
          default: new Footer({
            children: [
              new Paragraph({
                ...
                children: [
                  new TextRun({ text: "Generated by Scoutlytics — Powered by You.com APIs & Foxit PDF  |  Page ", size: 16, color: GRAY, font: "Segoe UI" }),
