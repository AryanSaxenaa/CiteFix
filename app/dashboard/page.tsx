...
export default function DashboardPage() {
  ...
  const statusBadge = (status: string) => {
    switch (status) {
      ...
      case "failed":
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-orange-500/10 text-orange-400 text-[10px] font-mono">
      ...
      <nav className="fixed top-6 left-0 right-0 z-50 flex justify-center px-4">
        <div className="bg-[#111]/80 backdrop-blur-xl border border-white/10 rounded-full pl-6 pr-2 py-2 flex items-center gap-8 shadow-2xl shadow-black/50">
          ...
          <div className="hidden sm:flex items-center gap-6">
            ...
            <span className="text-xs text-white font-medium border-b border-[#E8834A] pb-0.5">
              Dashboard
            </span>
          </div>
          <Link
      ...
      <main className="relative z-10 pt-32 pb-20 px-6 max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-[#0F0F0F] border border-white/10 rounded-lg pl-10 pr-4 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-[#E8834A]/50"
            />
          </div>
          <div className="relative">
            <select
              ...
              className="bg-[#0F0F0F] border border-white/10 rounded-lg px-4 py-2.5 text-sm text-gray-300 focus:outline-none focus:border-[#E8834A]/50 appearance-none pr-8"
            >
        ...
        {loading && (
          <div className="flex items-center justify-center py-20">
            <Loader2 className="w-6 h-6 text-[#E8834A] animate-spin" />
          </div>
        )}
        ...
        {!loading && jobs.length === 0 && (
          <div className="text-center py-20">
            ...
            <Link
              href="/analyze"
              className="inline-flex items-center gap-2 bg-[#E8834A] text-white px-6 py-3 rounded-lg text-sm font-semibold hover:bg-[#C2712F] transition-colors"
            >
              ...
              <div key={domain} className="mb-8 bg-[#0F0F0F] border border-white/10 rounded-xl p-5">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <Globe className="w-4 h-4 text-[#E8834A]" />
                  ...
                  <div className={`flex items-center gap-1 text-sm font-mono ${trend >= 0 ? "text-green-400" : "text-orange-400"}`}>
            ...
            {filteredJobs.map((job) => {
              ...
              return (
                <Link
                  ...
                  className="block bg-[#0F0F0F] border border-white/10 rounded-xl p-4 hover:border-[#E8834A]/30 transition-all group"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-4 sm:gap-6 shrink-0">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <ArrowRight className="w-4 h-4 text-gray-700 group-hover:text-[#E8834A] transition-colors" />
                      </div>
                    </div>
                  </div>
                  ...
                  {job.status === "complete" && (
                    <div className="mt-2 pt-2 border-t border-white/5 flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 text-[10px] text-[#E8834A] hover:text-orange-300 transition-colors bg-[#E8834A]/5 hover:bg-[#E8834A]/10 px-3 py-1.5 rounded-lg border border-[#E8834A]/20"
                      >
